...
async function main() {
  ...
  console.log("🚀 Starting precision QA audit across viewports...\n");
  ...
  for (const vp of VIEWPORTS) {
    ...
    if (vp.name === "375" || vp.name === "1440") {
      ...
      const fullPath = path.join(OUT_DIR, `precision-fullpage-${vp.name}.png`);
      ...
      for (const secId of SECTIONS) {
        ...
        if (secEl) {
          const secPath = path.join(OUT_DIR, `precision-${secId}-${vp.name}.png`);
          await secEl.screenshot({ path: secPath });
          console.log(`  📸 Saved section #${secId} at ${vp.name}px`);
        } else {
          console.warn(`  ⚠️ Section #${secId} not found on page!`);
        }
      }
    }

    // 4. Test Interactive Elements (Visual Diff Slider keyboard accessibility)
